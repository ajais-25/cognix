...
function TxnDescription({ txn }: { txn: CreditTransaction }) {
  ...
  if (txn.type === "deduction" && txn.tokenMeta)
    return (
      <span className="txn-desc">
        Query · {txn.tokenMeta.totalTokens.toLocaleString()} tokens{" "}
        <span className="txn-token-detail">
          ({txn.tokenMeta.promptTokens.toLocaleString()} in /{" "}
          {txn.tokenMeta.outputTokens.toLocaleString()} out)
        </span>
      </span>
    );
  if (txn.type === "topup")
    return <span className="txn-desc">Credit top-up</span>;
  if (txn.type === "bonus")
    return <span className="txn-desc">Bonus credits</span>;
      ...
      <div className="subpage-content">
        {!isLoggedIn && !authLoading ? (
          <div className="subpage-empty">
            ...
            <Link href="/sign-in" className="subpage-signin-btn">
              Sign in
            </Link>
          </div>
        ) : isLoading ? (
          ...
          <>
            {/* Balance card */}
            <div
              className={`credit-balance-card ${data?.lowBalance ? "credit-low" : ""}`}
            >
              ...
              <div className="credit-balance-right">
                {data?.lowBalance && (
                  <span className="credit-low-badge">⚠ Low balance</span>
                )}
                <button
                  id="add-credits-btn"
                  ...
                  {transactions.map((txn) => (
                    <div key={txn._id} className="txn-row">
                      <div className="txn-left">
                        <span
                          className={`txn-type-badge ${typeClass[txn.type]}`}
                        >
                          {typeLabel[txn.type]}
                        </span>
          ...
          <div className="acm-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="acm-header">
              ...
              <p className="acm-subtitle">
                Top up your account to keep using Cognix
              </p>
            </div>
            ...
            <div className="acm-footer">
              <button className="acm-cancel-btn" onClick={closeModal}>
                Cancel
              </button>
              <button
                id="pay-now-btn"
